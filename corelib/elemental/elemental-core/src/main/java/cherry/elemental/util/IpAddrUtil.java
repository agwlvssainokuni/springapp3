 ...
 * <ul>
 * <li>IPアドレス形式判定: IPv4 ({@link #isIpv4Addr(String)})、IPv6 ({@link #isIpv6Addr(String)})</li>
 * <li>IPv6アドレスの展開表記への変換 ({@link #decompressIpv6Addr(String)})</li>
 * <li>IPv6アドレスの圧縮表記 (RFC 5952) への変換 ({@link #compressIpv6Addr(String)})</li>
 * <li>IPアドレスの文字列表現から数値表現への変換: IPv4 ({@link #getIpv4AddrAsNumber(String)})、IPv6 ({@link #getIpv6AddrAsNumber(String)})</li>
 * <li>IPアドレスのマスクの生成: IPv4 ({@link #getIpv4AddrMask(int)})、IPv6 ({@link #getIpv6AddrMask(int)})</li>
 * <li>IPアドレスの数値表現から文字列表現への変換: IPv4 ({@link #getIpv4AddrFromNumber(BigInteger)} )、IPv6 (
 * {@link #getIpv6AddrFromNumber(BigInteger)})</li>
 * </ul>
 */
public class IpAddrUtil {
	 ...
	 * 
	 * @param maskLength ネットマスク長。0から32の範囲で指定する。
	 ...
	 * 
	 * @param prefixLength プレフィクス長。0から128の範囲で指定する。
	 ...
	 * 
	 * @param size IPアドレスのサイズ。
	 * @param length プレフィクス長。
	 * @return ビットマスクの数値表現。
	 */
	...
	private static Ipv6Fragment makeIpv6Fragment(String addr) {
		...
		if (addr.matches("^.*:[0-9]{1,3}(\\.[0-9]{1,3}){3}$")) {
			...
			if (addrv6.equals(":")) {
				...
				result.v6 = padding(6, new String[0], addrv6.substring(2).split(":"));
			} else if (addrv6.endsWith(":")) {
				result.v6 = padding(6, addrv6.substring(0, addrv6.length() - 1).split(":"), new String[0]);
			...
			if (addrv6.equals("::")) {
				...
				result.v6 = padding(8, new String[0], addrv6.substring(2).split(":"));
			} else if (addrv6.endsWith("::")) {
				result.v6 = padding(8, addrv6.substring(0, addrv6.length() - 2).split(":"), new String[0]);
			} else if (addrv6.indexOf("::") >= 0) {
				String[] part = addrv6.split("::");
	 ...
	 * @param prefix 先頭に格納する値を保持する配列。
	 * @param suffix 末尾に格納する値を保持する配列。
	 * @return 生成した配列。
	 */
	 ...
	 * 
	 * @param builder 文字列結合のためのStringBuilder。
	 * @param field IPv6アドレスの項を分解した文字列配列。
	 */
	private static void constructDecompressed(StringBuilder builder, String[] field) {
